...
if len(sys.argv) != 2:
    raise SystemExit("usage: route_rev_a_kl_astar.py BOARD.kicad_pcb")
...
ROUTES = [
    ...
    ("/BROTHER-CONNECTORS/EOL_R_N", (207.5, 132.025), (207.5, 130.91), "pullup-k"),
    ("/BROTHER-CONNECTORS/EOL_R_S", (207.5, 135.325), (205.5, 130.71), "pullup-l"),
]
...
def net_code(name):
    ...
    if c<=0: raise RuntimeError(f"missing net {name}")
    return c
...
def build_blocked(route_net):
    ...
    for item in board.GetTracks():
        same=item.GetNetCode()==own; is_via=isinstance(item,pcbnew.PCB_VIA)
        if is_via:
...
def add_track(a,b,name):
    tr=pcbnew.PCB_TRACK(board);tr.SetStart(pt(*a));tr.SetEnd(pt(*b));tr.SetLayer(LAYER);tr.SetWidth(iu(TRACK_W))
    n=net_obj(name)
...
for name,start,goal,label in ROUTES:
    ...
    report += [f"## {label}",f"- net: `{name}`",f"- start: {start}",f"- goal: {goal}",f"- cells: {len(raw)}",f"- segments: {len(points)-1}","- points: "+" -> ".join(f"({x:.3f},{y:.3f})" for x,y in points),""]

pcbnew.SaveBoard(str(PATH),board)
report_path=PATH.with_name("KH910_REV_A_KL_ASTAR_ROUTE.md")
report_path.write_text("\n".join(report)+"\n")
print("KL_ASTAR_ROUTE_OK",PATH)
print(report_path)
