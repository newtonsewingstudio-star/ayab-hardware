...
# Route order matters. L is attempted before K because the previous K-first
# In2 route was valid by itself but blocked L's only useful corridor.
ROUTES = [
    {
...
def bbox_mm(obj):
    ...
    if all(hasattr(r, n) for n in ("GetX", "GetY", "GetWidth", "GetHeight")):
        x = mm(r.GetX()); y = mm(r.GetY())
        w = mm(r.GetWidth()); h = mm(r.GetHeight())
        return x, y, x + w, y + h
...
def require_endpoint_via(name, pos, tol=0.02):
    ...
    for item in board.GetTracks():
        ...
        p = item.GetPosition()
        got = (mm(p.x), mm(p.y))
        if abs(got[0]-pos[0]) <= tol and abs(got[1]-pos[1]) <= tol:
            found.append(got)
    if len(found) != 1:
        raise RuntimeError(f"expected exactly one {name} via at {pos}, got {found}")
...
def add_track(a, b, name, layer):
    ...
    tr.SetStart(pt(*a)); tr.SetEnd(pt(*b))
    tr.SetLayer(layer); tr.SetWidth(iu(TRACK_W))
    n = net_obj(name)
...
def choose_candidate(route):
    ...
    for idx,(layer,start_stub,goal_stub) in enumerate(route["candidates"], start=1):
        ...
        if raw is not None:
            return idx,layer,start_stub,goal_stub,raw
...
for route in ROUTES:
    idx,layer,start_stub,goal_stub,raw = choose_candidate(route)
    points = simplify(raw,start_stub,goal_stub)
    add_track(route["actual_start"], start_stub, route["net"], layer)
    for a,b in zip(points,points[1:]):
        add_track(a,b,route["net"],layer)
    add_track(goal_stub,route["actual_goal"],route["net"],layer)
    ...
    report += [
        ...
        f"- net: `{route['net']}`",
        f"- candidate: {idx}",
        f"- layer: `{board.GetLayerName(layer)}`",
        f"- via start: {route['actual_start']}",
        f"- start stub: {start_stub}",
        f"- goal stub: {goal_stub}",
        f"- via goal: {route['actual_goal']}",
        f"- A* cells: {len(raw)}",
        f"- routed segments: {len(points)+1}",
