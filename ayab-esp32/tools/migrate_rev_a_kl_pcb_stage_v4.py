...
- move R213/R214 into the clear top-right strip at x=240 mm;
- feed both pull-ups from the existing +3V3 via at (238.69, 123.66);
- launch the two machine sides from well-spaced new vias at (241.6,118/120).
...
# New, verified-clear pull-up placement above/right of the ESP32 courtyard.
R213_NEW = (240.0, 118.0)
R214_NEW = (240.0, 120.0)
R213_P1 = (239.175, 118.0)
R213_P2 = (240.825, 118.0)
R214_P1 = (239.175, 120.0)
R214_P2 = (240.825, 120.0)
P3V3_VIA = (238.69, 123.66)
K_LAUNCH = (241.6, 118.0)
L_LAUNCH = (241.6, 120.0)
...
def postprocess(path: Path) -> None:
    ...
    extra = [
        # Existing +3V3 via at 238.69,123.66 feeds both resistors on F.Cu.
        core.k.segment(R213_P1, (238.69, 118.0), "F.Cu", n3, "kl-v5-r213-3v3-a"),
        core.k.segment(R214_P1, (238.69, 120.0), "F.Cu", n3, "kl-v5-r214-3v3-a"),
        core.k.segment((238.69, 118.0), P3V3_VIA, "F.Cu", n3, "kl-v5-3v3-spine"),
        # Short machine-side stubs to well-spaced launch vias.
        core.k.segment(R213_P2, K_LAUNCH, "F.Cu", nk, "kl-v5-r213-k"),
        core.k.segment(R214_P2, L_LAUNCH, "F.Cu", nl, "kl-v5-r214-l"),
        core.k.via(K_LAUNCH, nk, "kl-v5-k-launch"),
        core.k.via(L_LAUNCH, nl, "kl-v5-l-launch"),
    ]
    rend = core.root_end(text)
    text = text[:rend] + "\n\t" + "\n\t".join(extra) + "\n" + text[rend:]
    path.write_text(text, encoding="utf-8")
    print("KL_V5_PHYSICAL_CLEANUP_OK")
    print("KL_V5_PULLUPS", R213_NEW, R214_NEW)
    print("KL_V5_LAUNCH_VIAS", K_LAUNCH, L_LAUNCH)


def migrate(src: Path, dst: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="kh910-kl-v5-") as td:
        temp = Path(td) / "stage.kicad_pcb"
        core.migrate(src, temp)
        postprocess(temp)
        core.validate(temp)
        dst.write_text(temp.read_text(encoding="utf-8"), encoding="utf-8")
    print("KL_V5_ELECTRICAL_STAGE_OK", dst)
