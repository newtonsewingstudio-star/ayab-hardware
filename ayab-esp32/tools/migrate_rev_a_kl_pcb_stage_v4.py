...
- move R213/R214 onto the real top-right board area at x=260 mm, to the right
  of the top-edge notch and J801;
- feed both pull-ups from the existing +3V3 F.Cu endpoint at (257.13,120.25);
- launch the two machine sides from well-spaced new vias at (262,118/120).
...
# The board has a top-edge notch from approximately x=201.57..249.71 mm whose
# lower edge is y=121.679 mm. The earlier x=240 placement was physically inside
# that cutout. These coordinates are deliberately to the right of the notch and
# clear of J801's courtyard (which ends around x=253.85 mm).
...
P3V3_FEED = (257.13, 120.25)
K_LAUNCH = (262.0, 118.0)
L_LAUNCH = (262.0, 120.0)

# Vias generated by the original prototype plus the old level-shifter branch
# vias which become unnecessary after the machine/local split.
REMOVE_VIAS = {
...
def postprocess(path: Path) -> None:
    # Fail closed on the geometry mistake that triggered the prior routing loop.
    if R213_NEW[0] <= 249.71 or R214_NEW[0] <= 249.71:
        raise RuntimeError("K/L pull-ups must be to the right of the top-edge notch")
    ...
    # Apply edits first, then append the clean replacement copper.
    ...
    extra = [
        # Existing +3V3 F.Cu endpoint at 257.13,120.25 feeds both pull-ups.
        core.k.segment(P3V3_FEED, (258.2, 120.25), "F.Cu", n3, "kl-v6-3v3-feed-a"),
        core.k.segment((258.2, 120.25), R214_P1, "F.Cu", n3, "kl-v6-r214-3v3"),
        core.k.segment((258.2, 120.25), (258.2, 118.0), "F.Cu", n3, "kl-v6-3v3-rise"),
        core.k.segment((258.2, 118.0), R213_P1, "F.Cu", n3, "kl-v6-r213-3v3"),
        # Short machine-side stubs to well-spaced launch vias on real board area.
        core.k.segment(R213_P2, K_LAUNCH, "F.Cu", nk, "kl-v6-r213-k"),
        core.k.segment(R214_P2, L_LAUNCH, "F.Cu", nl, "kl-v6-r214-l"),
        core.k.via(K_LAUNCH, nk, "kl-v6-k-launch"),
        core.k.via(L_LAUNCH, nl, "kl-v6-l-launch"),
    ]
    rend = core.root_end(text)
    text = text[:rend] + "\n\t" + "\n\t".join(extra) + "\n" + text[rend:]
    path.write_text(text, encoding="utf-8")
    print("KL_V6_PHYSICAL_CLEANUP_OK")
    print("KL_V6_PULLUPS", R213_NEW, R214_NEW)
    print("KL_V6_LAUNCH_VIAS", K_LAUNCH, L_LAUNCH)
    print("KL_V6_3V3_FEED", P3V3_FEED)


def migrate(src: Path, dst: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="kh910-kl-v6-") as td:
        temp = Path(td) / "stage.kicad_pcb"
        core.migrate(src, temp)
        postprocess(temp)
        core.validate(temp)
        dst.write_text(temp.read_text(encoding="utf-8"), encoding="utf-8")
    print("KL_V6_ELECTRICAL_STAGE_OK", dst)
