...
The divider alone is suitable for detecting the nominal machine rail, but it is not sufficient input protection: an open R216 would leave the ESP32 input supplied through R215, and Espressif does not specify an allowable GPIO injection current for that condition. The release correction adds D205/D206 Schottky clamps to `+3V3` and ground plus C206 100 nF to ground. At a hypothetical 3.6 V node and nominal 47 kΩ R215, the resulting currents are 0.179 mA at 12 V, 0.243 mA at 15 V, and 0.774 mA at 40 V; these are conditional values, not upper bounds. R215 is rated from -55 °C to +155 °C with ±1% initial tolerance and ±100 ppm/°C TCR. Applying the worst-sign TCR across the full 130 °C maximum departure from the 25 °C nominal gives a conservative 45.919 kΩ minimum without inferring element temperature from ambient. With only the additional assumption that the protected node remains at or above ground, loose current upper bounds are therefore 0.261 mA, 0.327 mA, and 0.871 mA respectively. Neither calculation guarantees an ESP32-safe node voltage or surge performance: clamp forward voltage, 3.3 V rail behavior, leakage, polarity, temperature and transient duration remain part of assembly inspection and bring-up. C206 filters fast switching noise; D206 clamps negative excursions.
...
At the 0.65 A total limit, Q805's 70 mΩ stress estimate is 0.030 W and 0.046 V. The 1.0/1.27 mm shared switched-rail trunks and 0.8/0.4 mm vias carry at most the 0.65 A board limit; 0.25/0.35 mm branches are restricted to one approximately 86 mA coil or a low-current local load. This envelope assumes the controlled 1.6 mm four-layer stackup: 35 µm (1 oz nominal) outer copper, 17.5 µm (0.5 oz nominal) inner copper, and normally plated vias. The high-current routed trunks are on the outer layers; inner planes provide return/reference copper but are not credited as additional parallel current capacity. Sixteen-coil simultaneous operation and the 1.37 A all-coil case are prohibited on Rev A. Dummy-load bring-up must verify D601, Q805, regulator, track and via temperatures before this envelope can be expanded.
